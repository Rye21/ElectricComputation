 ...
 $DSN        = "HRDSQL2";    //Data sauce name
  ...
  if (! $con = odbc_connect($DSN, $DBUSER, $DBPASSWORD)) {
   ...
   if(!empty($_POST["code_number"]) ){
    ...
    $query = "SELECT customerCode,kihonsiyoCd,siyoCode,siyoDetailName FROM siyoDataBroker.dbo.tbl_siyo_kihon

	
	
	WHERE  customerCode = '$code_number' AND  kihonsiyoCd  = '0001' ORDER BY siyoCode  DESC   ";
